...
if [ "$1" == "build" ]; then
	...
	rm -rf $MPV_ANDROID/{mpv,.}/build $MPV_ANDROID/mpv/src/main/{libs,obj}
	exit 0
else
	exit 255
fi
...
ndk-build -C mpv/src/main -j$cores

targets=(mpv:assembleDebug)
if [ -z "$DONT_BUILD_RELEASE" ]; then
	targets+=(mpv:assembleRelease)
	[ -n "$BUNDLE" ] && targets+=(mpv:bundleRelease)
fi
./gradlew "${targets[@]}"

if [ -n "$ANDROID_SIGNING_KEY" ]; then
	cd "${MPV_ANDROID}/mpv/build/outputs/apk"
